...
public class SaveSystem : MonoBehaviour
{
    ...
    public void SaveData(string dataToSave, int slotNumber)
    {
        if(WriteToFile(saveName+slotNumber, dataToSave))
        {
            Debug.Log("Successfully saved data");
        }
    }

    public string LoadData(int slotNumber)
    {
        string data = "";
        if(ReadFromFile(saveName+slotNumber, out data))
        {
